...
function RetryPaymentButton({ orderId }: RetryPaymentButtonProps) {
  ...
  const handleRetryVNPay = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Lỗi khi thử lại thanh toán";
      setError(message);
      alert(message);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-3 mt-4 border-t border-[#4A4035] pt-4">
      <p className="text-sm text-[#E0B84F] mb-3">
        Phương thức thanh toán thất bại. Vui lòng thử lại:
      </p>
...
function MyOrders() {
  ...
  // Điểm tích lũy = 10% tổng tiền thanh toán cuối cùng của đơn hàng
  ...
  const getTimelineDotColor = (status: OrderStatus) => {
    switch (status) {
      ...
      case "CONFIRMED":
        return "border-sky-500 bg-sky-900";
      case "PREPARING":
        return "border-violet-500 bg-violet-900";
      case "SHIPPING":
        return "border-[#D4AF37] bg-[#8B6F47]/20";
      case "COMPLETED":
      case "DELIVERED":
        return "border-[#D4AF37] bg-[#8B6F47]/20";
      case "CANCELLED":
        return "border-[#D4AF37] bg-[#8B6F47]/10";
      default:
  ...
  if (!user) {
    return (
      <div className="mx-auto mt-24 w-full max-w-5xl px-4 py-14 text-center">
        <h1 className="mb-4 text-3xl font-['Noto_Serif'] font-bold text-[#F5F0EB]">
          Vui lòng đăng nhập
        </h1>
  ...
  if (loading) {
    return (
      <div className="mx-auto w-full max-w-5xl px-4 py-24 text-center text-[#E0B84F]">
        Đang tải...
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto mt-24 w-full max-w-6xl px-4 py-8 text-[#F5F0EB]">
      ...
      <div className="mb-10 text-center sm:text-left">
        <h1 className="text-4xl font-['Noto_Serif'] font-black text-[#F5F0EB]">
          Đơn hàng của tôi
        </h1>
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between bg-[#1A1A1A] p-6 rounded-2xl border border-[#1F1C18] shadow-lg">
        <div className="w-full sm:w-1/2">
          <label className="text-sm text-[#E0B84F] mb-2 block">
            Tìm kiếm sản phẩm
          </label>
        ...
        >
          ...
          >
            ...
            <div className="p-8 space-y-8">
              {/* TIMELINE */}
              <div>
                <h3 className="mb-6 font-['Noto_Serif'] font-bold text-lg text-[#E0B84F]">
                  Tiến trình đơn hàng
                </h3>
                ...
                <div className="flex overflow-x-auto justify-center items-center text-center pb-4 custom-scrollbar">
                  {selectedOrder.logs?.map((log, index, arr) => {
                    ...
                    return (
                      <div
                        key={log.id}
                        className="relative flex min-w-[120px] flex-col items-center text-center"
                      >
                        <div
                          className={`z-10 h-10 w-10 rounded-full border-[3px] shadow-lg ${getTimelineDotColor(
                            log.status
                          )}`}
                        />
              ...
              {/* ITEMS */}
              <div>
                <h3 className="mb-4 font-['Noto_Serif'] font-bold text-lg text-[#E0B84F]">
                  Sản phẩm
                </h3>
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="rounded-2xl bg-[#1F1C18] border border-[#4A4035] p-6">
                  <p className="text-xs uppercase tracking-wider text-[#E0B84F] mb-3">
                    Thông tin nhận hàng
                  </p>
                  ...
                  <p className="text-sm text-[#E0B84F] mt-1 mb-2">
                    {selectedOrder.phone}
                  </p>
                ...
                <div className="rounded-2xl bg-[#1F1C18] border border-[#4A4035] p-6">
                  <p className="text-xs uppercase tracking-wider text-[#E0B84F] mb-4">
                    Điểm thưởng
                  </p>
                  ...
                  <div className="space-y-3">
                    {selectedOrder.usedPoint > 0 && (
                      <div className="flex justify-between items-center pb-3 border-b border-[#4A4035]">
                        <span className="text-sm text-[#E0B84F]">
                          Điểm đã sử dụng
                        </span>

                        <span className="font-semibold text-[#F5F0EB]">
                          -{formatPoint(selectedOrder.usedPoint)} điểm
                        </span>
                      </div>
                    )}
                    ...
                    {getEarnedPoints(selectedOrder) > 0 && (
                      <div>
                        ...
                        <p className="text-xs text-[#E0B84F] mt-1">
                    ...
                    {isCompletedStatus(selectedOrder.status) && (
                ...
                <div className="rounded-2xl bg-[#1F1C18] border border-[#4A4035] p-6">
                  ...
                  <div className="space-y-4">
                    {selectedOrder.payments.map((payment) => (
                      <div
                        key={payment.id}
                        className="flex justify-between items-center bg-[#1A1A1A] p-4 rounded-xl border border-[#4A4035]"
                      >
                        <div>
              ...
              <div className="flex justify-between items-center border-t border-[#4A4035] pt-6 pb-2">
                <span className="text-xl font-['Noto_Serif'] text-[#F5F0EB]">
                  Tổng cộng
                </span>

                <span className="text-3xl font-bold text-[#D8A94A]">
