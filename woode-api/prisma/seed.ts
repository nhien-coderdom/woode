...
async function main() {
  ...
  const tableWork = await prisma.product.create({
    data: {
      ...
      modelUrl: '/wooden_table_02_1k.gltf',
  ...
  const tableCoffee = await prisma.product.create({
    data: {
      ...
      modelUrl: '/round_wooden_table_02_1k.gltf',
  ...
  const shelfWall = await prisma.product.create({
    data: {
      ...
      modelUrl: '/wooden_display_shelves_01_1k.gltf',
  ...
  const shelfBookcase = await prisma.product.create({
    data: {
      name: 'Tủ sách gỗ thông 4 tầng',
      price: 3200000,
      description: 'Tủ sách 4 tầng chất liệu gỗ thông tự nhiên, sơn PU chống ẩm. Nhiều ngăn tiện dụng, phù hợp phòng làm việc và phòng khách.',
      categoryId: shelfCategory.id,
      imageUrl: 'https://images.unsplash.com/photo-1594620302200-9a762244a156?w=800',
      modelUrl: '/wooden_display_shelves_01_1k.gltf',
  ...
  const lampFloor = await prisma.product.create({
    data: {
      ...
      modelUrl: '/brass_diya_lantern_1k.gltf',
